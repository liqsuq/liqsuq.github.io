...
ログイン時に特定のURLを全画面で表示するためのTipsです。Webフレームワークで実装したアプリケーションの操作
画面を
常に表示させた端末を作りたいといった用途の簡単な実装に使えると思います。どこかで使えそうなのでメモ。
...
sudo apt update
sudo apt install -y gnome-tweak-took ice
~~~
...
--kioskオプションを使うと起動時に全画面で表示し、<kbd>F11</kbd>での全画面の終了等の
...
だと<kbd>F11</kbd>は無効化されているのでシャットダウン等別の操作をする場合には
<kbd>Alt</kbd>+<kbd>F4</kbd>を入力して一度ブラウザを落としてください。
